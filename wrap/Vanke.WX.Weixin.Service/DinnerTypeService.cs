...
namespace Vanke.WX.Weixin.Service
{
    public class DinnerTypeService : GenericService<IDataContext, DinnerType, DinnerTypeModel>, IDinnerTypeService
    {
        ...
        protected override async Task UpdateEntityAsync(DinnerType entity)
        {
            await CheckTypeExist(entity);
        ...
        protected override Task RemoveEntityAsync(DinnerType entity)
        {
        ...
        {
            if (await UnitOfWork.Set<DinnerType>().AnyAsync(p => p.Type.Equals(entity.Type) && p.ID != entity.ID && p.Status!= DinnerTypeStatus.Removed))
            {
